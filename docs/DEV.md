...
Последняя: `0005_card_introduced_at.sql` — `cards.introduced_at timestamptz null`: когда карточке
...
заново: карточка открывается сразу ступенью узнавания. Перед ней:
