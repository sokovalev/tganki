...
Последняя: `0004_new_card_style.sql` — `users.new_card_style` (enum `reveal` / `choice`,
по умолчанию `choice`): чем спрашивать новую карточку, «выбор из четырёх» или «Показать ответ»
(§3.2 спеки). Варианты хранить негде и не нужно — они лежат в `sessions.queue[i].choice`,
это jsonb, миграции не требует. Там же живёт флаг `introduced` — карточку, которой уже показали
экран знакомства (§3.2), в этой сессии знакомят только один раз.
